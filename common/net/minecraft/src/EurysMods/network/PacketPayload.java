...
public class PacketPayload
{
	...
	public void append(PacketPayload other) {
		...
		if(other.intPayload.length > 0)
			this.intPayload = EurysCore.concat(this.intPayload, other.intPayload);
		if(other.floatPayload.length > 0)
			this.floatPayload = EurysCore.concat(this.floatPayload, other.floatPayload);
		if(other.stringPayload.length > 0)
			this.stringPayload = EurysCore.concat(this.stringPayload, other.stringPayload);
	...
	public void append(int[] other) {
		...
		this.intPayload = EurysCore.concat(this.intPayload, other);
	}
