...
public class TileEntityMTSign extends TileEntity
{
	...
	{
        String[] signLines = packet.getMtSignText();
	    for (int i = 0; i < 4; i++)
	    {
	        boolean flag = true;
	        if (signLines[i].length() > 15)
	        {
	            flag = false;
	        }
	        else
	        {
	            for (int l = 0; l < signLines[i].length(); l++)
	            {
	                if (ChatAllowedCharacters.allowedCharacters.indexOf(signLines[i].charAt(l)) < 0)
	                {
	                    flag = false;
	                }
	            }
	        ...
	        if (!flag)
	        {
	        	packet.payload.stringPayload[i] = "!?";
	        }
	    }
