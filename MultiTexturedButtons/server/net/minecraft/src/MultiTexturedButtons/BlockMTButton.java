...
public class BlockMTButton extends BlockContainer {
	...
	public boolean canPlaceBlockOnSide(World par1World, int par2, int par3,
			int par4, int par5) {
		return (par5 == 2 && par1World.isBlockSolidOnSide(par2, par3, par4 + 1,
				2))
	...
	public void onBlockPlaced(World par1World, int par2, int par3, int par4,
		...
		if (par5 == 2 && par1World.isBlockSolidOnSide(par2, par3, par4 + 1, 2)) {
			...
			var6 = 2;
				...
				&& par1World.isBlockSolidOnSide(par2 - 1, par3, par4, 5)) {
			var6 = 1;
		} else {
	...
	private int getOrientation(World par1World, int par2, int par3, int par4) {
		...
		if (par1World.isBlockSolidOnSide(par2, par3, par4 - 1, 3))
			return 3;
		if (par1World.isBlockSolidOnSide(par2, par3, par4 + 1, 2))
			return 4;
		return 1;
	}
	...
	public void setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess,
		...
		if (var6 == 1) {
			this.setBlockBounds(0.0F, var8, 0.5F - var10, var11, var9,
					0.5F + var10);
		} else if (var6 == 2) {
			this.setBlockBounds(1.0F - var11, var8, 0.5F - var10, 1.0F, var9,
					0.5F + var10);
		} else if (var6 == 3) {
			this.setBlockBounds(0.5F - var10, var8, 0.0F, 0.5F + var10, var9,
					var11);
		} else if (var6 == 4) {
			this.setBlockBounds(0.5F - var10, var8, 1.0F - var11, 0.5F + var10,
					var9, 1.0F);
		}
	}
	...
	public void onBlockRemoval(World par1World, int par2, int par3, int par4) {
		...
		TileEntityMTButton tileentitymtbutton = (TileEntityMTButton) par1World
		...
		if (tileentitymtbutton != null) {
			...
			if (itemDamage > -1) {
				ItemStack itemstack = new ItemStack(MTBCore.BlockMTButton, 1,
						itemDamage);
				EntityItem entityitem = new EntityItem(par1World, par2, par3,
						par4, new ItemStack(itemstack.itemID, 1,
	...
	public void updateTick(World par1World, int par2, int par3, int par4,
		...
		if (!par1World.isRemote) {
			...
			if ((var6 & 8) != 0) {
				par1World
						.setBlockMetadataWithNotify(par2, par3, par4, var6 & 7);
				par1World.notifyBlocksOfNeighborChange(par2, par3, par4,
						this.blockID);
				int var7 = var6 & 7;

				if (var7 == 1) {
					par1World.notifyBlocksOfNeighborChange(par2 - 1, par3,
							par4, this.blockID);
				} else if (var7 == 2) {
					par1World.notifyBlocksOfNeighborChange(par2 + 1, par3,
							par4, this.blockID);
				} else if (var7 == 3) {
					par1World.notifyBlocksOfNeighborChange(par2, par3,
							par4 - 1, this.blockID);
				} else if (var7 == 4) {
					par1World.notifyBlocksOfNeighborChange(par2, par3,
							par4 + 1, this.blockID);
