...
public class TileEntityMTSignRenderer extends TileEntitySpecialRenderer {
	...
	public void renderTileEntitySignAt(TileEntityMTSign mtstileentitysign,
			double d, double d1, double d2, float f, String texturePath) {
		Block block = mtstileentitysign.getBlockType();
		GL11.glPushMatrix();
		float f1 = 0.6666667F;
		if (block == MTSCore.mtSignPost) {
			GL11.glTranslatef((float) d + 0.5F, (float) d1 + 0.75F * f1,
					(float) d2 + 0.5F);
		...
		for (int k = 0; k < mtstileentitysign.mtSignText.length; k++) {
			...
			if (k == mtstileentitysign.mtsLineBeingEdited) {
				...
				fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2,
						k * 10 - mtstileentitysign.mtSignText.length * 5, j);
			} else {
				fontrenderer.drawString(s, -fontrenderer.getStringWidth(s) / 2,
						k * 10 - mtstileentitysign.mtSignText.length * 5, j);
			}
	...
	@Override
	public void renderTileEntityAt(TileEntity mtstileentity, double d,
