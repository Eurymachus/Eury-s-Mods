...
public class GuiEditMTSign extends GuiScreen {
	...
	protected void keyTyped(char c, int i) {
		...
		if (allowedCharacters.indexOf(c) >= 0
				&& mtsEntitySign.mtSignText[mtsEditLine].length() < 15) {
	...
	public void drawScreen(int i, int j, float f) {
		...
		drawCenteredString(fontRenderer, mtsScreenTitle, width / 2, 40,
				0xffffff);
		GL11.glPushMatrix();
		...
		TileEntityRenderer.instance.renderTileEntityAt(mtsEntitySign, -0.5D,
				-0.75D, -0.5D, 0.0F);
