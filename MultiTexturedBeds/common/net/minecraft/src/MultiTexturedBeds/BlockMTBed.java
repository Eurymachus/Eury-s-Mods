...
public class BlockMTBed extends BlockBed implements ITextureProvider, IContainer
{
	Class mtBedEntityClass;
	
    public BlockMTBed(int par1, Class bedClass, float hardness, boolean disableStats, boolean requiresSelfNotify)
    {
        super(par1);
	...
	public int getBlockTexture(IBlockAccess blockAccess, int x,
		...
		return this.getBlockTextureFromSideAndMetadata(l, blockAccess.getBlockMetadata(x, y, z), index);
	}
    ...
    public int getBlockTextureFromSideAndMetadata(int par1, int par2, int par3)
    {
        ...
        else
        {
            ...
            return isBlockFootOfBed(par2) ? (var4 == 2 ? index + 2 + 16 : (var4 != 5 && var4 != 4 ? index + 1 : index + 1 + 16)) : (var4 == 3 ? index - 1 + 16 : (var4 != 5 && var4 != 4 ? index : index + 16));
        }
    }
