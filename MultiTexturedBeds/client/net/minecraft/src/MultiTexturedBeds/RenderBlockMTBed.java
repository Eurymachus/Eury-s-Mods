...
public class RenderBlockMTBed {
	...
	public static boolean renderBlockMTBed(Block par1Block, int x, int y,
		...
		int mixedBrightness = par1Block.getMixedBrightnessForBlock(
				iblockaccess, x, y, z);
		...
		renderblocks
				.renderTopFace(par1Block, x, y, z, par1Block.getBlockTexture(
						renderblocks.blockAccess, x, y, z, 1));
		...
		if (blockTexture != 2
				&& (renderblocks.renderAllFaces || par1Block
						.shouldSideBeRendered(iblockaccess, x, y, z - 1, 2))) {
			tessellator.setBrightness(par1Block.minZ > 0.0D ? mixedBrightness
					: par1Block.getMixedBrightnessForBlock(iblockaccess, x, y,
							z - 1));
			tessellator.setColorOpaque_F(var11, var11, var11);
			renderblocks.flipTexture = var64 == 2;
			renderblocks.renderEastFace(par1Block, x, y, z,
					par1Block.getBlockTexture(iblockaccess, x, y, z, 2));
		}

		if (blockTexture != 3
				&& (renderblocks.renderAllFaces || par1Block
						.shouldSideBeRendered(iblockaccess, x, y, z + 1, 3))) {
			tessellator.setBrightness(par1Block.maxZ < 1.0D ? mixedBrightness
					: par1Block.getMixedBrightnessForBlock(iblockaccess, x, y,
							z + 1));
			tessellator.setColorOpaque_F(var11, var11, var11);
			renderblocks.flipTexture = var64 == 3;
			renderblocks.renderWestFace(par1Block, x, y, z,
					par1Block.getBlockTexture(iblockaccess, x, y, z, 3));
		}

		if (blockTexture != 4
				&& (renderblocks.renderAllFaces || par1Block
						.shouldSideBeRendered(iblockaccess, x - 1, y, z, 4))) {
			tessellator.setBrightness(par1Block.minZ > 0.0D ? mixedBrightness
					: par1Block.getMixedBrightnessForBlock(iblockaccess, x - 1,
							y, z));
			tessellator.setColorOpaque_F(var12, var12, var12);
			renderblocks.flipTexture = var64 == 4;
			renderblocks.renderNorthFace(par1Block, x, y, z,
					par1Block.getBlockTexture(iblockaccess, x, y, z, 4));
		}

		if (blockTexture != 5
				&& (renderblocks.renderAllFaces || par1Block
						.shouldSideBeRendered(iblockaccess, x + 1, y, z, 5))) {
			tessellator.setBrightness(par1Block.maxZ < 1.0D ? mixedBrightness
					: par1Block.getMixedBrightnessForBlock(iblockaccess, x + 1,
							y, z));
			tessellator.setColorOpaque_F(var12, var12, var12);
			renderblocks.flipTexture = var64 == 5;
			renderblocks.renderSouthFace(par1Block, x, y, z,
					par1Block.getBlockTexture(iblockaccess, x, y, z, 5));
		}
